...
public abstract class Activity_Main_Base2D_Sensor extends Activity_Main_Base2D implements SensorEventListener {
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        synchronized (this) {
            ...
            if (xy_angle < -2){
                Log.d("Activity_Main_Base2D", "up");
                //world.setPlayerSpeed(0, -1);
                //getWorld().setPlayerSpeed(0, xy_angle);
            }
            if (xz_angle > 2){
            	Log.d("Activity_Main_Base2D", "right");
            	//world.setPlayerSpeed(+1, 0);
            	//getWorld().setPlayerSpeed(xz_angle, 0);
            }
            if (xz_angle < -2) {
            	Log.d("Activity_Main_Base2D", "left");
            	//world.setPlayerSpeed(-1, 0);
            	//getWorld().setPlayerSpeed(xz_angle, 0);
            }
            if (xy_angle > 2)   {
            	Log.d("Activity_Main_Base2D", "down");
            	//world.setPlayerSpeed(0, +1);
            	//getWorld().setPlayerSpeed(0, xy_angle);
            }
